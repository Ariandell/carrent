...
const fragmentShaderSource = `
    ...
    void main() {
        ...
        // --- PRISM SETUP (Premium Depth - Large Background Element) ---
        float ar = u_resolution.x / u_resolution.y;
        float scale = 1.8; // Larger prism for background effect
        if (ar < 1.0) {
            scale = 2.5; // Still visible on mobile but as background
        }
        
        // Offset prism slightly to the right for better composition
        vec2 prismOffset = vec2(0.15, 0.0);

        float d = sdTriangle((uv - prismOffset) * scale, 1.0);
        ...
        if (d < 0.0) {
            ...
            vec2 facetUV = (uv - prismOffset) * scale;
